...
import { Service } from "./createSetting";
...
const UpdateSetting: React.FC<
 IModalUpdate
> = ({ open, onCancel, data }) => {
    const [code, setCode] = React.useState(data.id);
    const [name, setName] = React.useState(data.name);
    const [price, setPrice] = React.useState(data.price); 
    const [comboPrice, setComboPrice] = React.useState(data.comboPrice);
    const [comboNumber, setComboNumber] = React.useState(data.comboNumber);
    const [selected, setSelected] = React.useState(data.status);
    const [priceChecked, setPriceChecked] = React.useState(price ? true : false);
    const [comboChecked, setComboChecked] = React.useState(comboPrice ? true : false);
    ...
    const applicableTimeDefault = formatter.format(
        new Date(data.applicableDate)
      );
      const expiredTimeDefault = formatter.format(new Date(data.expiredDate));
    ...
    const handleOk = async () => {
      ...
      const combinedApplicableDateTime = applicableDate
        ...
        .set("second", applicableTime.second());
          await edit({data: {name: name, price:priceChecked ? price : null,comboPrice: comboChecked ? comboPrice : null,comboNumber:comboNumber,status:selected,applicableDate: combinedApplicableDateTime.toString(), expiredDate: combinedExpiredDateTime.toString()}, collectionName: "service", id: id})
      ...
      >
        <Space size={32} direction="vertical">
          ...
          <Space size={28}>
            <Row>
              <Col>
                ...
                <div className="flex-center">
                  <DatePickerCustom
                    defaultValue={dayjs(data.applicableDate)}
            ...
            <Row>
              <Col>
                ...
                <div className="flex-center">
                  <DatePickerCustom
                    defaultValue={dayjs(data.expiredDate)}
          ...
          <Space direction="vertical" className=" ">
            ...
            <Space>
              <Checkbox defaultChecked={priceChecked} className="checkbox" onChange={onChangePriceCheckbox}>
            ...
            <Space>
              <Checkbox defaultChecked={comboChecked} className="checkbox" onChange={onChangeComboCheckbox}>
                <Typography.Text className="text-normal medium-16 gray-brown opacity-7">
                  Combo vé với giá
                  </Typography.Text>
              </Checkbox>
